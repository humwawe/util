...
/**
 * suffixArray[i]: 排名第i的后缀是str.substring(sa[i])
 * lcpArray[i]: sa[i]和sa[i-1]代表后缀的最长公共前缀
 * 对已排好的sa来说，lcp(sa[i],sa[j]) = min(lcp(sa[i],sa[k]),lcp(sa[k],sa[j])) k在[i,j]之间
 *
 * @author hum
 */
public class SuffixArray {
  ...
  private static int[] saDoubling(int[] s) {
    ...
    int[] rnk = Arrays.copyOf(s, n);
    int[] tmp = new int[n];

    for (int k = 1; k < n; k *= 2) {
      final int _k = k;
      final int[] _rnk = rnk;
      IntBinaryOperator cmp = (x, y) -> {
        if (_rnk[x] != _rnk[y]) {
  ...
  private static void insertionsortUsingComparator(int[] a, IntBinaryOperator comparator) {
  ...
  private static void mergesortUsingComparator(int[] a, IntBinaryOperator comparator) {
    final int n = a.length;
